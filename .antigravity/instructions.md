...
- Every Prisma model MUST have `tenantId String` field with FK to Tenant
- Every backend endpoint MUST use `req.user.tenantId` (NEVER trust body)
- Index pattern: `@@index([tenantId, isDeleted])`
- Every query: `where: { tenantId, isDeleted: false, ... }`

### 3. Soft Delete (Mandatory)
- Pattern: `isDeleted Boolean @default(false)` + `deletedAt DateTime?`
