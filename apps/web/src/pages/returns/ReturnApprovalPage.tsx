...
import { Undo2, Check, X, Eye, Filter, Send } from 'lucide-react';
...
export function ReturnApprovalPage() {
  ...
  // Tek tek onaylama için — her satır kendi mutation'ını çağırır
  const approveOne = async (id: string) => {
    const m = useReturnAction(id);
    await m.mutateAsync({ action: 'approve' });
  };

  const rejectOne = async (id: string, reason: string) => {
    const m = useReturnAction(id);
    await m.mutateAsync({ action: 'reject', rejectionReason: reason });
  };
  ...
  const columns: DataTableColumn<ReturnListItem>[] = [
    ...
    { key: 'actions', label: 'İşlem', width: '200px', render: (r) => (
      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
        ...
        <RejectInlineButton returnId={r.id} onConfirm={(reason) => rejectOne(r.id, reason)} />
      </div>
    )},
  ];
    ...
    <div>
      <div className="space-y-4">
        ...
        <ConfirmModal
          ...
          description={`${confirmApprove?.returnNumber} numaralı iade onaylanacak. Onay sonrası tamamlanma adımı uygulanabilir.`}
          confirmText="Onayla"
          variant="info"
          onConfirm={async () => {
            if (confirmApprove) {
...
function RejectInlineButton({ returnId, onConfirm }: { returnId: string; onConfirm: (reason: string) => void | Promise<void> }) {
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-lg bg-surface p-5 shadow-xl">
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder="Red nedeni (en az 3 karakter)..." className="w-full rounded-md border border-outline bg-surface px-3 py-2 text-sm" />
