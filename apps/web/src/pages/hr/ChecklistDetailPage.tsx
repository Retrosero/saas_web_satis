...
export function OffboardingChecklistPage() {
  ...
  const { data: checklist, isLoading } = useOffboarding(id!);
  const updateItem = useUpdateOffboardingItem();
  const completeMut = useCompleteOffboarding();
  const cancelMut = useCancelOffboarding();
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="İşten Çıkış Süreci"
        description={checklist.employee?.fullName ?? 'Personel'}
        actions={
          <button
            onClick={() => navigate('/hr/checklists/offboardings')}
      ...
      <div className="rounded-lg border border-outline bg-surface p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm text-fg-muted">
              Çıkış Tarihi: {new Date(checklist.terminationDate).toLocaleDateString('tr-TR')}
              {checklist.reason && ` • Neden: ${checklist.reason}`}
            </p>
        ...
        <div className="mt-4">
          ...
          <div className="h-2 w-full overflow-hidden rounded-full bg-bg-subtle">
            <div className="h-full bg-primary transition-all" style={{ width: `${checklist.progress.percent}%` }} />
          </div>
        ...
        {checklist.notes && (
          <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">📝 {checklist.notes}</p>
        )}
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {item.isRequired && (
              <span className="rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700">Zorunlu</span>
            )}
          ...
          {item.notes && !notesOpen && <p className="mt-1 text-xs italic text-fg-muted">💬 {item.notes}</p>}
          {item.completedAt && (
            <p className="mt-0.5 text-[10px] text-green-700">✓ {new Date(item.completedAt).toLocaleString('tr-TR')}</p>
          )}
        </div>
        ...
        {!disabled && (
          <div className="flex shrink-0 gap-1">
            <button
              ...
              className={'rounded p-1.5 hover:bg-green-100 ' + (item.status === 'DONE' ? 'bg-green-100 text-green-700' : 'text-fg-muted')}
              title="Tamamlandı"
            >
            ...
            <button
              ...
              className={'rounded p-1.5 hover:bg-blue-100 ' + (item.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-700' : 'text-fg-muted')}
              title="Devam Ediyor"
            >
            ...
            <button
              ...
              className={'rounded p-1.5 hover:bg-red-100 ' + (item.status === 'BLOCKED' ? 'bg-red-100 text-red-700' : 'text-fg-muted')}
              title="Engellendi"
            >
            ...
            <button
              ...
              className={'rounded p-1.5 hover:bg-zinc-100 ' + (item.status === 'NOT_APPLICABLE' ? 'bg-zinc-200 text-zinc-700' : 'text-fg-muted')}
              title="Geçerli Değil"
            >
